...
class Chat extends Component {
  sendMessage(ev) {
    ...
    if(text) {
      client.service('crlistings').create({ text }).then(() => {
        input.value = '';
      });
    }
  ...
  componentDidMount() {
    ...
    client.service('crlistings').on('created', this.scrollToBottom);
    this.scrollToBottom();
  }

  componentWillUnmount() {
    // Clean up listeners
    client.service('crlistings').removeListener('created', this.scrollToBottom);
  }

  render() {
    const { users, crlistings } = this.props;
      ...
      <div className="flex flex-row flex-1 clear">
        ...
        <div className="flex flex-column col col-9">
          <main className="chat flex flex-column flex-1 clear" ref={main => { this.chat = main; }}>
            {crlistings.map(message => <div key={message._id} className="message flex flex-row">
